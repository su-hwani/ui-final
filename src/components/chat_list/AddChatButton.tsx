...
const AddChatButton: React.FC<AddChatButtonProps> = ({ onAddChat, onFullScreenToggle, isFullScreen }) => {
  return (
    <div className="mb-3 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {isFullScreen ? <BiCollapse className="h-5 w-5" /> : <BiExpand className="h-5 w-5" />}
        </button>
      </div>
    </div>
  );
